        ...
        (function (o, i) {
            ...
            return g({
              ...
              parsePrefix: (r) => {
                ...
                if (r.consume('StringValue'))
                  ...
                  let S = '';
                  const P = ['Identifier', '@', '/'];
                  for (; P.some((x) => r.consume(x)); ) (S += y.text), (y = r.lexer.current);
                  c = {
                    type: 'JsdocTypeSpecialNamePath',
                    value: S,
                    specialType: s,
            ...
            return g({
              ...
              parseInfix: (c, y) => {
                ...
                s && y.type === 'JsdocTypeNullable' && ((J = !0), (y = y.element)),
                  r && y.type === 'JsdocTypeReadonlyProperty' && ((S = !0), (y = y.element));
                ...
                if (
                  ...
                  if (le(y) && !e) throw new d(y);
                  P.consume(':');
                  let x;
                  y.type === 'JsdocTypeStringValue' && (x = y.meta.quote);
                  const re = P.parseType(m.KEY_VALUE);
                  return (
                    c.acceptLexerState(P),
                    {
                      type: 'JsdocTypeObjectField',
                      key: le(y) ? y : y.value.toString(),
                      right: re,
                      optional: J,
                      readonly: S,
                      meta: { quote: x },
          ...
          const Le = [
            ...
            en = g({
              ...
              parsePrefix: (e) => {
                ...
                return (
                  e.consume('is'),
                  { type: 'JsdocTypeAsserts', left: t, right: w(e.parseIntermediateType(m.INFIX)) }
                );
              },
            });
          ...
          const nn = g({
            ...
            an = g({
              ...
              parsePrefix: (e) => (
                e.consume('readonly'),
                { type: 'JsdocTypeReadonlyProperty', element: e.parseType(m.KEY_VALUE) }
              ),
            }),
            on = g({
          ...
          function ge(e, t, r, s, c) {
            ...
            for (const T of y) {
              const J = e[T];
              if (J !== void 0)
                if (Array.isArray(J)) for (const S of J) ge(S, e, T, s, c);
                else ge(J, e, T, s, c);
            }
            c?.(e, t, r);
          }
  ...
  Y = l((n) => {
    ...
    switch ((typeof o < 'u' && (i.raw = o), n.name)) {
      ...
      case 'signature':
        return { ...i, ...On(n) };
      case 'union':
        return n.elements?.every(kn)
          ? { ...i, name: 'enum', value: n.elements?.map(xn) }
...
function tt({ fence: n = '```' } = {}) {
  ...
  return l(function (i) {
    let p = [[]],
      u = !1;
    for (let d of i)
      Vn.test(d.tokens.description) && !u ? p.push([d]) : p[p.length - 1].push(d),
...
function bt(n, a) {
  if (n != null) {
    let { value: o } = n;
    if (!ae(o)) return Et(o) ? E(a?.name, o) : E(o);
  }
  return null;
}
...
function W(n, a) {
  if (a?.includesJsDoc) {
    let { description: o, extractedTags: i } = a;
